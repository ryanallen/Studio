...
1. **Task checklist (required)** - Before running any workflow or delegated task, run verifier with [verify-task](.claude/skills/verify-task/SKILL.md) to create or update .tmp/task-checklist.md. Do not run any flow steps until this is done.
2. **Document every skill** - After running each skill in the flow, update the current task section in `.tmp/task-checklist.md`: strikethrough that skill line and add a brief note. Do not run the next skill until the checklist is updated. This records every skill you do and prevents skipping steps (e.g. document-voice at the end of a refine).
